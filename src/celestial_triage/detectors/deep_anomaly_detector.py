...
def evaluate(features: dict) -> tuple[float, list[str]]:
    ...
    low_conf = clamp01(1.0 - features.get("avg_class_confidence", 0.0))
    ...
    return clamp01(score), [
        f"avg_class_confidence={features.get('avg_class_confidence', 0):.2f}",
        f"mag_jump={mag_jump:.2f}",
        f"odd_combo={odd_combo:.2f}",
    ]
